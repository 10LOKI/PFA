<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl md:text-3xl font-heading font-black text-[var(--neon-magenta)] uppercase tracking-wider drop-shadow-[0_0_15px_#FF00FF]">
            <span class="text-glow-magenta">CREATE REWARD</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12 relative z-10">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="glass-panel border-2 border-[var(--neon-magenta)] p-8 relative">
                ...
                <form action="{{ route('rewards.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf

                    {{-- Title --}}
                    <div>
                        <x-input-label for="title" :value="__('Title')" />
                        <x-text-input id="title" name="title" type="text" class="block w-full" :value="old('title')" required autofocus />
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                                  class="w-full px-4 py-3 bg-[var(--void-bg)] border-2 border-[var(--neon-magenta)] text-[var(--chrome-text)] font-mono focus:outline-none focus:shadow-[var(--glow-magenta)] transition-all duration-300 resize-none">{{ old('description') }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="points_cost" name="points_cost" type="number" min="1" class="block w-full" :value="old('points_cost')" required />
                            <x-input-error :messages="$errors->get('points_cost')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="stock" :value="__('Stock (leave empty for unlimited)')" />
                            <x-text-input id="stock" name="stock" type="number" min="0" class="block w-full" :value="old('stock')" />
                    ...
                    <div>
                        ...
                        <select id="min_grade" name="min_grade" required class="block w-full px-4 py-3 bg-[var(--void-bg)] border-2 border-[var(--neon-magenta)] text-[var(--chrome-text)] font-mono focus:outline-none focus:border-[var(--neon-cyan)] focus:ring-2 focus:ring-[var(--neon-cyan)] focus:ring-opacity-30 transition-all duration-200">
                            ...
                            <option value="pilier" {{ old('min_grade') == 'pilier' ? 'selected' : '' }}>PILLAR</option>
                            <option value="ambassadeur" {{ old('min_grade') == 'ambassadeur' ? 'selected' : '' }}>AMBASSADOR</option>
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="expires_at" name="expires_at" type="date" class="block w-full" :value="old('expires_at')" />
